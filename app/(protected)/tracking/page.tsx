...
export default function TrackingPage(): React.ReactElement {
  ...
  const { result, error } = usePaginatedList<{
    ...
    totalPages: number;
  }>("/api/tracking", page, debouncedSearch);
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-sm text-muted-foreground">
                  {entry.title}
                  {entry.totalChapter !== null || entry.lastChapterRead !== null
                    ? ` \u00b7 ${t("chapterProgress", { read: entry.lastChapterRead ?? 0, total: entry.totalChapter ?? "?" })}`
                    : ""}
                </p>
              </div>
              {entry.trackingUrl && /^https?:\/\//.test(entry.trackingUrl) && (
                <a
