...
import { updatesPageRepository } from "@/lib/repositories/updates-repository";
...
const PAGE_SIZE = PAGE_SIZES.updates;
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const { entries, total } = await updatesPageRepository.listPage({
    userId: user.id,
    page,
    pageSize: PAGE_SIZE,
    search,
